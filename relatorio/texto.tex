...
\begin{minipage}[t]{0.45\textwidth}
    ...
    % Acumulador? $\infty$ em C, \texttt{uint8\_t} e \mintinline{c}{uint16_t}. Row MAJOR, $n \cdot m = 0 \leftrightarrow n = 0 \lor m = 0$. \alert{kw FIM?}. Testes: $C_{i, j} \leq 99 < 2^{8}$ e $n \leq 100$, então
    % \[
    %     \sum_{i = 1}^n \max_{1 \leq j \leq m}\left\{C_{i, j}\right\} \leq \sum_{i = 1}^n 99 \leq 9900 < 2^{16}
    % \]
\end{minipage}%
\hfill%
