...
\def\chave#1{[#1]}
\begin{tikzpicture}
    \draw[step=\passo*2cm,color=gray] (-5*\passo,0*\passo) grid (5*\passo,4*\passo);
    ...
    \draw [color=gray,thick,->] (0*\passo, 0 *\passo - 0.5*\passo) -- (0*\passo, -4*\passo + 0.5*\passo) node[midway, right, color=darkgray] {$j \gets j + 1$};

    \draw[step=\passo*2cm,color=gray] (-5*\passo,-8*\passo) grid (5*\passo,-4*\passo);
